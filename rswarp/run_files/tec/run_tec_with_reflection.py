...
def main(x_struts, y_struts, V_grid, grid_height, strut_width, strut_height,
         ...
         run_id, channel_width=100e-9, diff_ref_prob=0.0, spec_ref_prob=0.0,
    ...
    reflections = True
    srefprob = spec_ref_prob
    drefprob = diff_ref_prob
    reflection_scheme = 'uniform'
    
    top.uxbirthpid = nextpid()
    top.uybirthpid = nextpid()
    top.uzbirthpid = nextpid()
    ...
    if install_grid:
        ...
        plate = ZPlane(zcent=zplate, voltage=ACCEL_VOLTS)
        if install_circuit:
            circuit = ExternalCircuit(top, solverE, total_rho, collector_voltage, cathode_area * 1e4, plate, debug=False)
            plate.voltage = circuit
    ...
    efficiency.tec_parameters['phi_em'][0] -= ACCEL_VOLTS
    
    from rswarp.particlecollector.reflector_tools import get_reflector_emission, get_species_current, get_total_current
    try:
    ...
    # TODO: Need to add ACCEL VOLTS back in here to get actual work function. This really needs to be cleaned up.
    param_copy = deepcopy(efficiency.tec_parameters)
    param_copy['phi_em'][0] = efficiency.tec_parameters['phi_em'][0] + ACCEL_VOLTS
    efficiency.tec_parameters['P_em'][0] = efficiency.calculate_analytic_power_flux(measured_charge[scraper_dictionary['source']] / efficiency.tec_parameters['run_time'][0],
                                                                                    **param_copy)
    
# Testing flux calculation from birth pid info
#     ux_lost = measurement_beam.getuxbirth(lost=True)
#     uy_lost = measurement_beam.getuybirth(lost=True)
#     uz_lost = measurement_beam.getuzbirth(lost=True)
#     ux_live = measurement_beam.getuxbirth(lost=False)
#     uy_live = measurement_beam.getuybirth(lost=False)
#     uz_live = measurement_beam.getuzbirth(lost=False)
#     bg = np.sqrt(ux_lost**2 + uy_lost**2 + uz_lost**2) / clight
#     KE =np.sum(Converter(betagamma=bg)()['kenergy']) * np.abs(measurement_beam.charge)
#     print('total KE', KE)
#     print('power', KE / efficiency.tec_parameters['run_time'][0] / efficiency.tec_parameters['A_em'][0])
#     total_current_flux = (ux_lost.size + ux_live.size) * measurement_beam.sw * np.abs(measurement_beam.charge) / efficiency.tec_parameters['run_time'][0] / efficiency.tec_parameters['A_em'][0]

#     total_energy = KE + total_current_flux * param_copy['phi_em'][0]
#     print('total', total_energy)
#     # backwards_power = measured_charge[scraper_dictionary['source']] / efficiency.tec_parameters['run_time'][0] * (param_copy['phi_em'][0] + )
