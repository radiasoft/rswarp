...
#Add patches
solverE.addchild(mins=[-0.5e-6, -0.5e-6, 0], maxs=[0.5e-6, 0.5e-6, z_cone * 2.], refinement = [4,4,4])
...
#Install sphere
e1 = emitters.spherical(temperature = source_temperature, 
                x_offset = 0.0, y_offset = 0.0, voltage = 0.0, N_particles = 500)
...
np.save('beam_dump_%s_%s.npy' % (int(ANODE_VOLTAGE), int(source_temperature)), particles)
...
np.save('axis_fields_%s_%s.npy' % (int(ANODE_VOLTAGE), int(source_temperature)), np.column_stack([x, y, z, Ex_tip, Ey_tip, Ez_tip, phi_tip, Bx_tip, By_tip, Bz_tip]))
...
np.save('target_particles_%s_%s.npy' % (int(ANODE_VOLTAGE), int(source_temperature)), particles)