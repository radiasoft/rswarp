...
path_to_rswarp=r"/Users/yhu/Documents/Work/RadiaSoft/rswarp"
...
def main(V_grid, rho_ew, T_em, phi_em, T_coll, phi_coll, rho_cw, gap_distance, rho_load,
         run_id,
         channel_width=6.9e-6,
         dx_want=1e-7, dy_want=1e-7, dz_want=1e-7,
         install_grid=True, grid_file=None, grid_scraper="regular",
         injection_type=2, magnetic_field=0.0, random_seed=True, max_wall_time=1e9,
    ...
    if install_grid:
        accel_grid = STLconductor(grid_file, voltage=V_grid, normalization_factor=dz, condid=1)

    # grid height = z_min of stlconductor grid - z_min of simulation box
    grid_height = np.amin(accel_grid.surface.vertices, axis=0)[2]
    grid_height -= Z_MIN
    grid_height /= gap_distance
    if grid_height < 0.0 or grid_height > 1.0:
        raise Exception("Illegal grid position")
    efficiency.tec_parameters["grid_height"][0] = grid_height
    ...
    if install_grid:
        if grid_scraper == "regular":
            ...
            fastscraper_flag = 1
            collectlpdata_flag = 0
        else:
            raise Exception("Illegal grid_scraper value")

        installconductor(accel_grid)
        installconductor(source, dfill=largepos)
        installconductor(plate, dfill=largepos)
        scraper = ParticleScraper([accel_grid],
                                  lfastscraper=fastscraper_flag,
                                  lcollectlpdata=collectlpdata_flag,
                                  lsaveintercept=0)
